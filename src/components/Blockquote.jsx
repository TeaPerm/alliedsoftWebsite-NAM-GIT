...
function BlockquoteWithImage({ author, children, className, image }) {
  return (
    ...
    >
      <blockquote className="col-span-2 text-xl/7 text-secondary sm:col-span-7 sm:col-start-6 sm:row-start-2">
      ...
      <figcaption className="text-sm text-accent-950 sm:col-span-7 sm:row-start-3 sm:text-base">
  ...
  return (
    <Border position="left" className={clsx('pl-8', className)}>
      <figure className="text-sm">
        <blockquote className="text-secondary [&>*]:relative [&>:first-child]:before:absolute [&>:first-child]:before:right-full [&>:first-child]:before:content-['“'] [&>:last-child]:after:content-['”']">
          {typeof children === 'string' ? <p>{children}</p> : children}
        </blockquote>
        <figcaption className="mt-6 font-semibold text-accent-950">
